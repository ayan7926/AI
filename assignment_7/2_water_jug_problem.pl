...
water_jug(X,Y):-
				...
				(X=:=3,Y=:=3,nl,write('4L:4 & 3L:2 (Action: Pour water from 3L jug to 4L jug until 4L jug is fill.)'), XX is X+1, YY is Y-1, water_jug(XX,YY));
				...
				(X=:=4,Y=:=1,nl,write('4L:2 & 3L:3 (Action: Pour water from 4L jug to 3L jug until 3L jug if full.)'), XX is X-2, YY is Y+2, water_jug(XX,YY));
